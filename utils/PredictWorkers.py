...
def predict_jobs_with_batch_predictor(
    predictor,
    input_raster: str,
    jobs: List[dict],
    config,
):
    cfg = _config_from_dict(dict(getattr(config, 'to_dict', lambda: {})()))
    stats = {
        'read_s': 0.0,
        'read_data_s': 0.0,
        'read_mask_s': 0.0,
    ...
    infer0 = time.perf_counter()
    pred_cores = _predict_batch(raw_tiles, raw_masks, predictor, cfg)
    stats['infer_s'] = time.perf_counter() - infer0
    return pred_cores, stats
