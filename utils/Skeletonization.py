...
def find_segments(pred, config, profile) -> (List[Part], List[Tuple[int]]):
    ...
    segments, skel = find_skeleton_segments(
        skel, end_nodes, math.floor(min_length / px_size), padding, config=config
    )
...
def _trace_chain(start: Tuple[int, int], neighbor: Tuple[int, int], skel: np.ndarray,
                 node_set: set, visited_edges: set) -> List[Tuple[int, int]]:
    path = [start, neighbor]
...
def _trace_loop(seed: Tuple[int, int], skel: np.ndarray, visited_edges: set) -> List[Tuple[int, int]]:
...
def find_skeleton_segments(
    ...
    remaining = [tuple(map(int, p)) for p in np.argwhere(skel_bool & (~out_skel))]
